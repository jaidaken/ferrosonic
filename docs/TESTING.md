...
- **app-input batch (input.rs/input_library/input_queue/input_songs, 212 mutants, 49 survivors): NAV SLICE done, rest intentionally left.** USER DECISION (2026-06-15): engine is deeply tested; the 49 input survivors are lower-stakes TUI-interaction logic (key-dispatch arms, cursor arithmetic, focus routing) already heavily tested but under-asserted. Chose "nav slice then stop". DONE: input_library song-pane cursor 170/171/220/221 (Down/Up move selected_song by exactly one; the real artist/album right-pane bug area) in `tests/input_library_tree_nav.rs`. LEFT AS-IS (real but lower-stakes, not chased): the other ~45 input survivors (key-dispatch arm deletes, focus-guard `==` mutants, compound `&&`->`||`) + the unmeasured app files (mouse*/event_pump/cava_pipe/mod/lifecycle/state/page_state/spawn_daemon/client_state), mpris/server, and ui-remaining widgets/pages. content-driven call: engine bugs cost data/playback/security; chrome bugs cost a keystroke.
- **util batch (secret/io_util/config/error/proc_util, 141 mutants, 17 survivors): security ones tested.** KILLED: config as_on_disk 137/141 (password must NOT land in config when password_file set; `||`->`&&` + dropped-`!` would leak it; `tests/config_on_disk_security.rs` via save_to_file), write_password_file_atomic (0600 perms + content + missing-parent creation, was untested), secret from_bytes 38 + PartialEq 154 (only doc-tested / untested before; `tests/secret_construction.rs`). EQUIV/log-only: config:340 unknown-key warn (delete-`!` warns on known vs unknown, log only), io_util:27 age `>`->`>=` (timing boundary), secret:173 Visitor::expecting (error-message text). SKIP (thin std delegation, trait tested via FailingFs not RealFs): io_util RealFs 51/68/70/75. KNOWN-OPEN: secret:132 Drop zeroize (freed-memory, unobservable in a deterministic test).
- **audio batch (mpv+pipewire, 99 mutants, 7 survivors): 1 cheap kill, rest equiv/seam.** KILLED: mpv.rs:501 get_bit_depth `||`->`&&` (a "float" format must map to 32-bit; existing tests only used s24/s16; added float case to `mpv_controller_full.rs`). EQUIV (exclusions): pipewire.rs:195 `rate>0`->`>=` (u32, both yield "0" at 0), 213 `!is_finished()` delete (log-only). KNOWN-OPEN (seam/real-process): mpv.rs:225 tear_down_connection + 573 Drop (process/connection teardown, needs a real mpv; relates to `SUBPROCESS-LEAK-BUG.md`), pipewire.rs:84 runtime-flavor guard (runtime-dependent), 210 `<`->`<=` deadline (timing boundary, unobservable).
- **subsonic batch (3 files, 58 mutants, 11 survivors): real ones tested.** REAL+tested: auth.rs generate_salt 33/34/36 (salt charset+range; the existing assertion was a DOC test, which nextest/mutation skips, so added `tests/subsonic_auth.rs` nextest-visible salt property test), client.rs 218/259/314 `status != "ok"` -> `==` in get_artist/get_album/get_playlist (error response would return Ok-empty not Err; existing error tests asserted state-empty which is true either way, so added direct `is_err()` tests in `subsonic_client_endpoints.rs`). SKIP: client.rs:47 base_url (trivial `#[doc(hidden)]` getter, no src callers, test-seam only).
...
- **playback_tick.rs: 16 survivors -> 11 killed + 1 equiv + 4 known-open** (55 mutants). killed: decide boundaries 140/141/142/145 (exact-value inline tests in `playback_tick_tests`), tr arithmetic 83 + has_next 86 (AdvanceEarly-vs-Preload queue_position discriminator) + tick_fetch 302 (`tests/daemon_playback_tick.rs`). equiv: 291 backfill-0.0 (exclusions doc). CLOSED 2026-06-22: the just_loaded 1500ms + bump_preload_due 5s boundaries were extracted to pure `within_just_loaded_window` / `preload_due` (parameterized on `now`), boundary-tested in the `boundary_tests` module, mutant-verified (`<`->`<=`, `>=`->`>` both fail). no clock-injection seam needed.
