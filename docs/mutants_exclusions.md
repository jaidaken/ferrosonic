...
- `core.rs` `prebuffer_and_load` cancel-path `gate.disarm()` / `slot_cleaner.disarm()` (3 sites: loop-top, pre-loadfile, post-loadfile). removing any is equivalent. each Buffered task owns a PER-TASK loading `Arc` and cancel `Arc`, and a superseding task replaces both slots BEFORE flipping this task's cancel. so by the time a cancelled task's `PrebufferGate`/`CancelSlotCleaner` Drop runs, its loading `Arc` is no longer the one in `prebuffer_loading` (the gate clears a detached `Arc`, harmless) and `prebuffer_cancel` holds the newer task's cancel (the cleaner's `Arc::ptr_eq` is false, so it skips). disarm only skips the redundant Drop work; it is an optimization, not a correctness gate. verified: removing the loop-top pair leaves `tests/buffered_playback.rs` + `core::guard_tests` green. the guard LOGIC mutants (disarm methods, Drop `armed` check, `Arc::ptr_eq`) ARE killed by `core::guard_tests`.
