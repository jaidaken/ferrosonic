...
fn next_manual_off_advances_then_stops_at_end() {
    ...
    assert_eq!(mode.next_manual(2, 3), None, "Off does not wrap on manual Next");
}
...
fn next_manual_one_still_advances_on_manual_skip() {
    ...
    assert_eq!(mode.next_manual(2, 3), Some(0), "repeat-One wraps on manual Next");
}
...
fn next_handlers_return_none_on_empty_queue() {
    for mode in [RepeatMode::Off, RepeatMode::One, RepeatMode::All] {
        assert_eq!(mode.next_manual(0, 0), None, "{:?} manual on empty queue", mode);
